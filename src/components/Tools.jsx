    ...
    <section className="bg-base py-20 px-6">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl md:text-4xl font-bold text-primary mb-12">
          AI tools <span className="text-accent">offered to Founding Cohort</span>
        </h2>

        <div className="flex flex-col lg:flex-row gap-8 bg-base/50 rounded-3xl overflow-hidden border border-primary/10 backdrop-blur-sm">
          
          {/* LEFT SECTION: Term Selection */}
          <div className="w-full lg:w-1/3 bg-base/50 border-r border-primary/10">
            {Object.keys(COURSE_DATA).map((term) => (
              <button
                key={term}
                onClick={() => setActiveTerm(term)}
                className={`w-full text-left p-6 transition-all border-b border-primary/5 last:border-0 flex items-center justify-between group ${
                  activeTerm === term ? "bg-primary text-base" : "text-primary/70 hover:bg-primary/5"
                }`}
              >
                <div className="flex flex-col">
                  <span className={`text-xs uppercase tracking-widest font-bold mb-1 ${
                    activeTerm === term ? "text-primary/60" : "text-accent"
                  }`}>
                    {term}
                  </span>
                ...
                {activeTerm === term && (
                  <motion.div layoutId="arrow" className="text-primary font-bold">→</motion.div>
                )}
              </button>
            ))}
          </div>
          ...
          <div className="w-full lg:w-2/3 p-8 md:p-12 min-h-[500px]">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="mb-12">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {COURSE_DATA[activeTerm].tools.map((tool) => (
                      <div 
                        key={tool.name} 
                        className="bg-primary/5 border border-primary/10 rounded-xl p-4 flex items-center gap-3 hover:bg-primary/10 transition-colors"
                      >
                        <img src={tool.logo} alt={tool.name} className="w-6 h-6 object-contain grayscale brightness-200" />
                        <span className="text-primary text-sm font-semibold">{tool.name}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Techniques Grid */}
                <div>
                  <h4 className="text-accent text-xs uppercase tracking-[0.2em] font-black mb-6">Techniques</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    {COURSE_DATA[activeTerm].techniques.map((tech) => (
                      <div key={tech.name} className="flex items-start gap-4 group">
                        ...
                        <div>
                          <p className="text-primary font-bold text-base leading-tight">{tech.name}</p>
                        </div>
                      </div>
                    ))}
                  </div>
