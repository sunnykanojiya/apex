  ...
  return (
    <FadeInSection className="px-4 py-14 sm:px-6 lg:px-8" delay={0.3}>
      <div className="mx-auto max-w-6xl">
        <h2 className="text-3xl font-bold tracking-tight text-primary sm:text-4xl">Proof That Compounds</h2>
        <div className="mt-7 grid grid-cols-1 gap-4 md:grid-cols-2">
          {testimonials.map((item) => (
            <blockquote key={item.name} className="glass rounded-2xl p-6">
              <p className="text-muted">"{item.quote}"</p>
              <footer className="mt-4 text-sm text-muted">{item.name}</footer>
            </blockquote>
          ))}
        </div>
      </div>
