...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="bg-base py-20 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl">
        <h2 className="text-3xl font-extrabold tracking-tight text-primary sm:text-4xl mb-12 text-center">
            ...
            return (
              <motion.div 
                ...
                className={`rounded-xl border-2 transition-all duration-300 ${
                  isOpen ? 'border-primary shadow-md' : 'border-primary/10'
                }`}
              >
                <button
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <span className={`font-semibold text-lg ${isOpen ? 'text-primary' : 'text-primary/70'}`}>
                      {item.q}
                    </span>
                  </div>
                  ...
                  <div className="flex-shrink-0">
                    {isOpen ? (
                      <X className="h-6 w-6 text-accent transition-transform rotate-0" />
                    ) : (
                      <Plus className="h-6 w-6 text-accent/70 transition-transform" />
                    )}
                  </div>
                </button>
                  ...
                  {isOpen && (
                    ...
                    >
                      <div className="px-14 pb-6">
                        ...
                        {item.cta && (
                          <button className="mt-4 bg-accent text-white px-5 py-2 rounded-full text-sm font-medium hover:bg-accent/90 transition-colors shadow-sm">
                            {item.cta}
                          </button>
                        )}
                      </div>
